...
public class SqlLogInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		try {
			...
			log.info("SQL：" + sqlCost + "ms > " + size + " > " + sql);
		}
	}
	 ...
	 *                                 处理tk Example 参数的sql
	 */
	private String handleExampleParameter(String sql, Example example, List<ParameterMapping> parameterMappingList)
			throws IllegalAccessException, IntrospectionException, InvocationTargetException {
	...
	private String handlePageExampleParameter(String sql, PageExample example,
			List<ParameterMapping> parameterMappingList)
			throws IllegalAccessException, IntrospectionException, InvocationTargetException {
		final List<Example.Criteria> oredCriteria = example.getOredCriteria();
		sql = handleCriteria(sql, parameterMappingList, oredCriteria);
		sql = sql.replaceFirst("\\?",
				example.getFirstPageHelper().toString() + "," + example.getSecondPageHelper().toString());
		return sql;
	}

	private String handleCriteria(String sql, List<ParameterMapping> parameterMappingList,
	...
	private String handleCommonParameter(String sql, List<ParameterMapping> parameterMappingList,
		...
		if (isPrimitiveOrPrimitiveWrapper(parameterObjectClass) || parameterObject instanceof BigDecimal) {
	...
	private boolean isPrimitiveOrPrimitiveWrapper(Class<?> parameterObjectClass) {
		return parameterObjectClass.isPrimitive() || (parameterObjectClass.isAssignableFrom(Byte.class)
				|| parameterObjectClass.isAssignableFrom(Short.class) || parameterObjectClass
				.isAssignableFrom(Integer.class) || parameterObjectClass.isAssignableFrom(Long.class)
				|| parameterObjectClass.isAssignableFrom(Double.class) || parameterObjectClass
				.isAssignableFrom(Float.class) || parameterObjectClass.isAssignableFrom(Character.class)
	...
	private PageExample tryPageExample(Object o) {
		try {
			...
			return PageExample.builder().firstPageHelper(firstPageHelper).oredCriteria(oredCriteria)
